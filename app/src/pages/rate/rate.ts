...
export class RatePage {
  ...
  constructor(public navCtrl: NavController, public navParams: NavParams) {
    ...
    for (const roundAnswersidx in this.answers) {
      const roundAnswers = this.answers[roundAnswersidx] 
      for (const answeridx in roundAnswers) {
        const answer = roundAnswers[answeridx]
        if (answer.userId === this.playerUserId) answer.text = this.pubquiz.rounds[this.round].playerAnswer[roundAnswersidx]
        // console.log(answer)
      }
    }
  }
